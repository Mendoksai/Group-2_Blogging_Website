    ...
    <div class="py-10 px-4 sm:px-10">
        <div class="max-w-4xl mx-auto">
            <div class="overflow-hidden shadow-sm rounded-lg">
                <div class="text-white my-3 bg-gray-700 p-1 rounded-lg shadow-lg">
                    <!-- User Information Section -->
                    <div class="flex flex-col md:flex-row items-center md:items-center p-3">
                        <!-- Profile Picture -->
                        <div class="flex items-center justify-center mt-3 md:mt-0">
                            @if (empty($user->profile_picture))
                                <img class="w-36 h-36 rounded-full bg-gray-300"
                                    src="{{ asset('imgs/default_profile.jpg') }}" alt="Default Profile Pic">
                            @else
                                <img class="w-36 h-36 rounded-full bg-gray-300 object-cover"
                                    src="{{ asset($user->profile_picture) }}" alt="Profile Pic">
                            @endif
                        </div>

                        <!-- User Details -->
                        <div class="text-center md:text-left ml-6">
                            <p class="capitalize font-black text-4xl">{{ $user->first_name }} {{ $user->last_name }}</p>
                            <p class="capitalize font-italic">{{ $user->account_role }}</p>
                        </div>
                    </div>

                    <!-- Empty Profile Picture Message -->
                    @if (empty($user->profile_picture))
                        <div class="mx-2 my-1 rounded-lg bg-blue-400 flex flex-row items-center p-3 lg:mx-10">
                            <svg class="w-12 ml-2 h-12 sm:w-10 sm:h-10 text-white mr-3" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M12 2.625c-5.17 0-9.375 4.206-9.375 9.375 0 5.17 4.206 9.375 9.375 9.375 5.17 0 9.375-4.206 9.375-9.375 0-5.17-4.206-9.375-9.375-9.375Zm0 3.844a1.219 1.219 0 1 1 0 2.437 1.219 1.219 0 0 1 0-2.437Zm2.25 10.593h-4.125a.75.75 0 1 1 0-1.5h1.313v-4.124h-.75a.75.75 0 1 1 0-1.5h1.5a.75.75 0 0 1 .75.75v4.874h1.312a.75.75 0 1 1 0 1.5Z">
                                </path>
                            </svg>
                            <div>
                                <h1 class="text-2xl font-bold mb-1">Info</h1>
                                <p class="text-sm">Profile picture is empty. Upload a profile picture by editing in
                                    the navigation menu "Edit Profile".</p>
                            </div>
                        </div>
                    @endif

                    <!-- User Details -->
                    <div class="text-base font-bold py-4 mx-4">
                        <p class="mb-2">Student/Instructor ID: <span
                                class="font-normal">{{ $user->student_instructor_id }}</span></p>
                        <p class="mb-2">Email: <span class="font-normal">{{ $user->email }}</span></p>
                        <p class="">Degree Name: <span
                                class="uppercase font-normal">{{ $user->degree_name }}</span></p>
                    </div>
                </div>

                <!-- Post Section -->
                <h2 class="text-2xl text-white font-bold mb-4 text-center uppercase">Posts</h2>
